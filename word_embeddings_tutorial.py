...
class CBOW_0h(nn.Module):

    def __init__(self, vocab_size, embedding_dim):
        super(CBOW_0h, self).__init__()
        self.embeddings = nn.Embedding(vocab_size, embedding_dim)
    ...
    def forward(self, inputs):
        ...
        out = F.relu(self.linear1(embeds))
        nll_probs = F.log_softmax(out, dim=1)
        return nll_probs

### code refer to https://github.com/wjdghks950/CBOW_Word2Vec/blob/master/CBOW_pytorch.py
class CBOW_1h(nn.Module):

    def __init__(self, vocab_size, embedding_dim):
        super(CBOW_1h, self).__init__()
        self.embeddings = nn.Embedding(vocab_size, embedding_dim)
...
model = CBOW_0h(len(vocab), EMBEDDING_DIM)
...
for epoch in range(100):
    ...
    for context, target in data:
        ...
        model.zero_grad()
        nll_probs = model(context_vector)
        loss = loss_function(nll_probs, torch.tensor([word_to_ix[target]], dtype=torch.long))
...
context_vector = make_context_vector(context, word_to_ix)
probs = model(context_vector).data.numpy()
print('Raw text: {}\n'.format(' '.join(raw_text)))
print('Test Context: {}\n'.format(context))
max_idx = np.argmax(probs)
print('Prediction: {}'.format(ix_to_word[max_idx]))
